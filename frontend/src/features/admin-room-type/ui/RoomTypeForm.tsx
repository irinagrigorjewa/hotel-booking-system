...
import {
  ...
  toRoomTypeFormValues,
  type RoomTypeFormValues,
} from '../model/mapRoomTypeForm'
...
export const RoomTypeForm = ({
  ...
  const {
    ...
    defaultValues: toRoomTypeFormValues(initialRoomType),
  })

  useEffect(() => {
    reset(toRoomTypeFormValues(initialRoomType))
  }, [initialRoomType, reset])

  const submit = async (values: RoomTypeFormValues): Promise<void> => {
    await onSubmit(values.name.trim())
    if (!initialRoomType) {
      reset(emptyRoomTypeFormValues())
    }
  }
